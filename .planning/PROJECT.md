...
---
*Last updated: 2026-05-28 — Phase 22 complete (HYG-02/03/06/07/08 + PERF-01/02/03/04/07; 10 requirements closed across 6 plans). `JobsManager.tsx` 2,067 → 1,494 LOC (-573 / -27.7%, under the <1,500 ROADMAP gate). `<RecordSaleModal>` extracted with verbatim Convert-from-Quote `db.transaction` (Pitfall 2 preserved); `useCustomerPicker` hook consumed by RecordSaleModal + PrintQuoteModal; `<SaleRow>` extracted with smoke test; `SearchIcon` deduped into `ui/icons/` barrel; `breakEvenMap` `useMemo` Map gives O(1) lookup per row; `calculateMarketplaceFee` lifted to module scope + memoized; `useDynamicRowHeight` keyed on `searchQuery` for CustomerLibrary; `useAllSales()` hook lifted from duplicate `useSales()` subscription. Tests 428 passing + 1 todo (28 files); 26 new test cases across `useCustomerPicker`, `RecordSaleModal`, `SaleRow`. Verifier: 11/11 must-haves verified. Code review: 0 critical / 3 warnings (WR-01 dead `userProfile` prop · WR-02 `RecordSaleModal` hydration `useEffect` could clobber in-progress EDIT/CONVERT state on concurrent `useLiveQuery` emission · WR-03 module-scope `*Ran` flags pre-existing) / 4 info — advisory, no blockers. Manual UAT approved all 5 visual contracts (break-even pill, recent sales accordion, RecordSaleModal create/edit/convert, Convert-from-Quote tx, CustomerLibrary search). v1.3 Hardening milestone status: 7 of 8 phases complete (18, 19, 20, 21, 22, 24, 25); remaining: Phase 23 (Test coverage hardening) + a newly-filed reconciliation phase (break-even formula divergence between CostCalculator's "Total Fixed Costs" widget and JobsManager's pill — surfaced during Phase 22 UAT, separate scope). Next: open `/gsd:discuss-phase` for the reconciliation, then Phase 23.*
