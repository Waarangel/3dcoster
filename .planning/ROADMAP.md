...
Plans:
- [ ] 22.1-01-PLAN.md — Schema extension (PrintJob.fixedCostsAtSave) + reconcileFixedCostsAtSave helper (Path B faithful reconstruction) + 4 Vitest cases + useJobs() module-flag wire-up (DATA-07) — Wave 1
- [ ] 22.1-02-PLAN.md — CostCalculator writes snapshot on Save+Update + JobsManager computeBreakEvenInfo numerator widened + 3 PERF-08 round-trip tests (TDD: tests first, formula change second) (PERF-08) — Wave 2, depends_on [22.1-01]
- [ ] 22.1-03-PLAN.md — Phase 22 code-review carry-overs: WR-01 dead-prop removal (userProfile from RecordSaleModalProps + JobsManager callsite) + WR-02 hydration dep array narrowed with Pitfall 3 comment + WR-02 regression test (TDD ordering) — Wave 2, depends_on [22.1-01]
...
| 22.1. Break-even formula reconciliation | 0/4 | Not started | — |
