...
- [ ] **Asset Library: filter by tag + stock status** - The Asset Library only filters by category today. Add a secondary filter for (a) the existing `Asset.tags` and (b) derived stock state — **Out of stock** / **Low stock** (the StockBadge state) / **In stock**. Note "out of stock" is a *derived* status from the stock ledger, not a literal tag, so expose it as its own facet alongside user tags. UI: a tag/status filter row beneath the category chips (combines with the active category + search). No schema change — `tags[]` and stock derivation already exist. *(User-requested, 2026-06-26)*
- [ ] **RecordSaleModal: surface `etsy_offsite_ad` in the marketplace picker** - The fee engine (`computeMarketplaceFee`) already supports Etsy Offsite Ads, but the Record Sale modal's `availableMarketplaces` list doesn't offer it, so sellers can't record that fee on a sale. Small UI gap — add the option (and its % to the fee summary line). *(Source: v1.9 release review, 2026-07-03)*
